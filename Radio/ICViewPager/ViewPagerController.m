...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
#pragma mark-----------如果上面的滚动的lable出现不了，但是存在，改下面的Y
//     frame.origin.y = self.tabLocation ? 0 : self.view.frame.size.height - self.tabHeight;//在这里我将0改成了64
    frame.origin.y = self.tabLocation ? 64 : self.view.frame.size.height - self.tabHeight;
    frame.size.width = self.view.bounds.size.width;
    frame.size.height = self.tabHeight - 10;//这里也是改的
    _tabsView.frame = frame;
    
    frame = _contentView.frame;
    frame.origin.x = 0.0;//这里如果变成100就可以做菜单了
